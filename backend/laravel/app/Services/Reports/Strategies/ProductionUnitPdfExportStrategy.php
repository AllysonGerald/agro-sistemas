...
class ProductionUnitPdfExportStrategy implements ExportStrategyInterface
{
    ...
    private function addDataRow($sheet, array $item, int $row, int $index): void
    {
        ...
        $sheet->setCellValue('C' . $row, $coordenadas);
    ...
    private function applyFormatting($sheet, array $data): void
    {
        ...
        $sheet->getColumnDimension('B')->setWidth(15); // Área (ha)
        $sheet->getColumnDimension('C')->setWidth(20); // Coordenadas
        $sheet->getColumnDimension('D')->setWidth(25); // Propriedade
        ...
        $sheet->getStyle('A5:E' . $lastRow)->getBorders()->getOutline()
            ->setBorderStyle(Border::BORDER_MEDIUM)
            ->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('2E7D32'));

        // Bordas internas
        $sheet->getStyle('A5:E' . $lastRow)->getBorders()->getInside()
            ->setBorderStyle(Border::BORDER_THIN)
            ->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('E0E0E0'));
    }
}
