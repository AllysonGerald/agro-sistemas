...
class ProductionUnitExcelExportStrategy implements ExportStrategyInterface
{
    public function export(array $data, string $filename, array $options = []): BinaryFileResponse
    {
        ...
        // Adicionar cabeçalho
        $this->addHeader($sheet, $options['title'] ?? 'Relatório de Unidades de Produção', $options['subtitle'] ?? '');
    ...
    private function addDataRow($sheet, array $item, int $row, int $index): void
    {
        ...
        // Nome da Cultura
        $sheet->setCellValue('A' . $row, $item['cultura'] ?? '');
        $this->formatDataCell($sheet, 'A' . $row, $bgColor, $textColor, 14);

        // Área
        $sheet->setCellValue('B' . $row, $item['area_ha'] ?? '0,00');
        $this->formatDataCell($sheet, 'B' . $row, $bgColor, $textColor, 12, Alignment::HORIZONTAL_CENTER);

        // Coordenadas
        $sheet->setCellValue('C' . $row, $item['coordenadas'] ?? '-');
        $this->formatDataCell($sheet, 'C' . $row, $bgColor, $textColor, 10, Alignment::HORIZONTAL_CENTER);

        // Propriedade
        $sheet->setCellValue('D' . $row, $item['propriedade'] ?? '');
        $this->formatDataCell($sheet, 'D' . $row, $bgColor, $textColor, 12);

        // Município
        $sheet->setCellValue('E' . $row, $item['municipio'] ?? '');
        $this->formatDataCell($sheet, 'E' . $row, $bgColor, $textColor, 12);

        // Data de Cadastro
        $sheet->setCellValue('F' . $row, $item['data_cadastro'] ?? '');
        $this->formatDataCell($sheet, 'F' . $row, $bgColor, $textColor, 10, Alignment::HORIZONTAL_CENTER);
    ...
    private function applyFormatting($sheet, array $data): void
    {
        ...
        $sheet->getColumnDimension('B')->setWidth(15); // Área
        $sheet->getColumnDimension('C')->setWidth(20); // Coordenadas
        $sheet->getColumnDimension('D')->setWidth(30); // Propriedade
        $sheet->getColumnDimension('E')->setWidth(20); // Município
        $sheet->getColumnDimension('F')->setWidth(15); // Data Cadastro
