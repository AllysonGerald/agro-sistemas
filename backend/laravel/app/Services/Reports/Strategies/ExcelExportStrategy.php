...
class ExcelExportStrategy implements ExportStrategyInterface
{
    ...
    {
        // Título principal com design moderno (igual ao PDF)
        $sheet->setCellValue('A1', $title);
        $sheet->mergeCells('A1:L1'); // Expandido para cobrir todas as colunas
        ...
        $sheet->getStyle('A1')->getFont()
            ->setBold(true)
            ->setSize(24) // Aumentado para 24px
            ->setColor($whiteColor);
        ...
        // Subtítulo e data (igual ao PDF)
        $sheet->setCellValue('A2', $subtitle . ' • Gerado em: ' . now()->format('d/m/Y H:i:s'));
        $sheet->mergeCells('A2:L2'); // Expandido para cobrir todas as colunas
        $sheet->getStyle('A2')->getFont()
            ->setSize(14) // Aumentado para 14px
            ->setItalic(true)
            ->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('666666'));
        $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getRowDimension(2)->setRowHeight(30); // Aumentado para 30px

        // Linha separadora elegante (igual ao PDF)
    ...
    {
        $startRow = 5; // Ajustado para começar após a linha separadora
        $currentRow = $startRow;

        // Adicionar cabeçalhos das colunas (igual ao PDF moderno)
        if (isset($data[0]) && is_array($data[0])) {
            $this->addColumnHeaders($sheet, $currentRow);
            $currentRow += 2; // Espaçamento entre cabeçalho e dados
    ...
    private function addColumnHeaders($sheet, int $row): void
    {
        ...
        foreach ($headers as $col => $header) {
            ...
            $sheet->getStyle($col . $row)->getFill()
                ->setFillType(Fill::FILL_SOLID)
                ->getStartColor()->setRGB('388E3C'); // Verde médio (igual ao PDF)
            $sheet->getStyle($col . $row)->getAlignment()
                ->setHorizontal(Alignment::HORIZONTAL_CENTER)
                ->setVertical(Alignment::VERTICAL_CENTER);

            // Bordas bem definidas para cabeçalhos (igual ao PDF)
    ...
    private function formatDataCell($sheet, string $cell, string $bgColor, string $textColor, int $fontSize, string $alignment = Alignment::HORIZONTAL_LEFT): void
    {
        ...
        // Bordas bem definidas para células de dados (igual ao PDF)
    ...
    {
        // Ajustar largura das colunas para layout moderno (igual ao PDF)
        ...
        // Adicionar bordas externas bem definidas (igual ao PDF)
