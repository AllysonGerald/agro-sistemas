...
class ProducerExcelExportStrategy implements ExportStrategyInterface
{
    public function export(array $data, string $filename, array $options = []): BinaryFileResponse
    {
        ...
        // Adicionar cabeçalho
        $this->addHeader($sheet, $options['title'] ?? 'Relatório de Produtores Rurais', $options['subtitle'] ?? '');
    ...
    private function addHeader($sheet, string $title, string $subtitle): void
    {
        ...
        // Forçar cor branca de forma mais explícita
        $whiteColor = new \PhpOffice\PhpSpreadsheet\Style\Color();
        ...
        $sheet->getRowDimension(1)->setRowHeight(50); // Aumentado para 50px
    ...
    private function addDataRow($sheet, array $item, int $row, int $index): void
    {
        ...
        // CPF/CNPJ
        $sheet->setCellValue('B' . $row, $item['cpf_cnpj'] ?? '');
        $this->formatDataCell($sheet, 'B' . $row, $bgColor, $textColor, 10);

        // E-mail
        $sheet->setCellValue('C' . $row, $item['email'] ?? '');
        $this->formatDataCell($sheet, 'C' . $row, $bgColor, $textColor, 10);

        // Telefone
        $sheet->setCellValue('D' . $row, $item['telefone'] ?? '');
        $this->formatDataCell($sheet, 'D' . $row, $bgColor, $textColor, 10);

        // Endereço
        $sheet->setCellValue('E' . $row, $item['endereco'] ?? '');
        $this->formatDataCell($sheet, 'E' . $row, $bgColor, $textColor, 10);

        // Total de Propriedades
        $propriedades = $item['total_propriedades'] ?? 0;
        $sheet->setCellValue('F' . $row, $propriedades);
        $this->formatDataCell($sheet, 'F' . $row, $bgColor, $textColor, 12, Alignment::HORIZONTAL_CENTER);

        // Total de Hectares
        $hectares = $item['total_hectares'] ?? '0,00';
        $sheet->setCellValue('G' . $row, $hectares . ' ha');
        $this->formatDataCell($sheet, 'G' . $row, $bgColor, $textColor, 12, Alignment::HORIZONTAL_CENTER);

        // Data de Cadastro
        $dataCadastro = $item['data_cadastro'] ?? '';
        $sheet->setCellValue('H' . $row, $dataCadastro);
        $this->formatDataCell($sheet, 'H' . $row, $bgColor, $textColor, 10, Alignment::HORIZONTAL_CENTER);
    ...
    private function applyFormatting($sheet, array $data): void
    {
        ...
        $sheet->getColumnDimension('C')->setWidth(30); // E-mail
        $sheet->getColumnDimension('D')->setWidth(15); // Telefone
        $sheet->getColumnDimension('E')->setWidth(35); // Endereço
        $sheet->getColumnDimension('F')->setWidth(12); // Propriedades
        $sheet->getColumnDimension('G')->setWidth(15); // Hectares
        $sheet->getColumnDimension('H')->setWidth(15); // Data Cadastro
        ...
        $sheet->getStyle('A5:H' . $lastRow)->getBorders()->getOutline()
            ->setBorderStyle(Border::BORDER_MEDIUM)
            ->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('2E7D32'));

        // Bordas internas
        $sheet->getStyle('A5:H' . $lastRow)->getBorders()->getInside()
            ->setBorderStyle(Border::BORDER_THIN)
            ->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('E0E0E0'));
    }
}
